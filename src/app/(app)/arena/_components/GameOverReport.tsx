...
function TeamScoreCard({ teamName, score, isWinner }: { teamName: string, score: number, isWinner: boolean }) {
    return (
        <Card className={cn("text-center", isWinner ? 'border-green-500 bg-green-500/10' : 'border-destructive bg-destructive/10')}>
            <CardHeader className="p-4">
                <CardTitle className={cn("text-2xl", isWinner ? 'text-green-600' : 'text-destructive')}>{teamName} Team</CardTitle>
            </CardHeader>
...
export function GameOverReport({ battleData, teamBattleData, player, onFindNewMatch }: GameOverReportProps) {
    ...
    // --- Team Battle Report ---
    if (teamBattleData) {
        const isPlayerOnTeam1 = teamBattleData.team1.some(p => p.id === player.id);
        const playerTeam = isPlayerOnTeam1 ? 'team1' : 'team2';
        const outcome = teamBattleData.winnerTeam === 'draw' ? 'draw' : (teamBattleData.winnerTeam === playerTeam ? 'win' : 'loss');

        let title = "Team Match Over";
        let message = "";
        let icon = <Users className="h-16 w-16 text-muted-foreground mx-auto mb-4" />;
        
        if (outcome === 'win') {
            title = 'Team Victory!';
            message = `Your team was victorious!`;
            icon = <CheckCircle className="h-16 w-16 text-green-500 mx-auto mb-4" />;
        } else if (outcome === 'loss') {
            title = 'Team Defeat';
            message = `The opposing team won this time.`;
                ...
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                           <TeamScoreCard 
                                teamName="Blue"
                                score={teamBattleData.team1Score}
                                isWinner={teamBattleData.winnerTeam === 'team1'}
                           />
                           <TeamScoreCard 
                                teamName="Red"
                                score={teamBattleData.team2Score}
